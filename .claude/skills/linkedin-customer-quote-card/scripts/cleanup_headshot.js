#!/usr/bin/env node
/*
 * linkedin-customer-quote-card — Gemini Image headshot cleanup
 *
 ...
 * without altering the subject's likeness. Used by step 3 of the
 ...
 * no vignette) keyed to that hex. Re-run cleanup per variant if you use this.
 *
...
function backdropInstruction(gradient) {
  if (!gradient) {
    return "a flat, slightly desaturated dark backdrop matching the card palette";
  }
  const hex = gradient.trim();
  if (!/^#[0-9a-fA-F]{6}$/.test(hex)) {
    throw new Error(
      `--gradient must be a 6-digit hex like "#0F1924" (got "${gradient}")`,
    );
  }
...
async function cleanupHeadshot({
  ...
  if (!apiKey) {
    throw new Error(
      "GEMINI_API_KEY env var is not set. The linkedin-customer-quote-card skill cannot run headshot cleanup without it. Set GEMINI_API_KEY locally or as a secret on the routine.",
    );
  }
