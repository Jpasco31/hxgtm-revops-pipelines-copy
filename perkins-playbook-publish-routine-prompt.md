# Perkins Playbook — Publish-to-Production Routine Prompt

> Runtime prompt for the Perkins Playbook **stage-2 / "Finalize & Publish Assets"**
> Claude cloud routine. This file IS the routine's instructions — the routine
> config references it verbatim. Keep it lean and imperative; the system-design
> reference lives in `PERKINS_PLAYBOOK.md`.
>
> **Companion:** `perkins-playbook-routine-prompt.md` is the **stage-1** routine
> (the `Generate Assets` trigger). This file is its stage-2 counterpart and mirrors it in
> structure, dispatcher mechanics, context loading, output rendering, and
> failure handling. The only differences are the trigger status, the wiring
> columns read, the output heading, and the status transitions — all called out
> below.

## Purpose

Autonomously process the **publish / finalization** step of a Perkins card.
When a card enters Status = `Finalize & Publish Assets`, resolve its Playbook relation
to determine the **stage-2** skill(s), Content Type, and Guidance Type, then
execute the card — launching a **subagent per card** to keep context clean.
Also supports single-task mode, where a Notion webhook (or a manually-supplied
card page ID) triggers a run for one specific card. After each card the
orchestrator appends a dated output block under the Outputs section of Step 2:
Finalize & Publish Assets and sets status per the transitions below.

This routine is a **generic dispatcher**. It runs whatever skills the linked
Playbook row's `Finalize & Publish Assets Skill` column specifies — it hard-codes no skill names,
...
This is the **stage-2** routine. It shares the Perkins Job Board, the Playbooks
Overview, and every card page with the **stage-1** routine
(`perkins-playbook-routine-prompt.md`). Because both stages read the same card and
write to the same page, the boundary between them is enforced here, not assumed. A
single card surfaces **both** stages' wiring as rollups (`Generate Assets Skill` **and** `Finalize & Publish Assets Skill`,
`Generate Assets Content Type` **and** `Finalize & Publish Assets Content Type`, `Generate Assets Guidance Type` **and** `Finalize & Publish Assets Guidance Type`).
Seeing the stage-1 columns is normal; acting on them is the bug this section exists
to prevent.

**This routine OWNS, and may only touch:**

| Boundary | Stage-2 (this routine) owns |
| --- | --- |
| Wiring columns | `Finalize & Publish Assets Skill`, `Finalize & Publish Assets Content Type`, `Finalize & Publish Assets Guidance Type`, `Finalize & Publish Assets Custom Instructions` (the stage-2 columns) |
| Trigger status | `Finalize & Publish Assets` |
| Processing status | `In progress` (set on the pipeline `Status` at claim — Step 4a) |
| Output section | the Outputs section of Step 2: Finalize & Publish Assets in the card body |
| Completed status | `Completed` |

**Status-to-body-step mapping.** This routine fires on cards with status
`Finalize & Publish Assets` (a kanban column label) and writes outputs to the *Outputs
section of Step 2: Finalize & Publish Assets* (a heading inside the card body).
It also **reads** stage-1's deliverables from the *Outputs section of Step 1:
Generate Assets* as input context. The lane, the wiring columns, and the body
step now deliberately share the **Finalize & Publish Assets** phrase — they all
...
**FORBIDDEN — never read for wiring, merge, fall back to, dispatch, or execute:**

- The stage-1 `Generate Assets Skill`, `Generate Assets Content Type`, `Generate Assets Guidance Type`, `Generate Assets Custom Instructions`
  columns (stage-1's wiring).
- Writing to the Outputs section of Step 1: Generate Assets. (Stage-2 **reads**
  it as input context — see below — but never appends to, edits, or reorders it,
  and never executes the skills the `Generate Assets Skill` column lists.)
- Any skill that is not present in this card's `Finalize & Publish Assets Skill` column.

Build the skill list **solely** from the `Finalize & Publish Assets Skill` column. If a skill would only
appear via the stage-1 `Generate Assets Skill` column, it does not run in this routine — full
stop. A dispatched skill list that contains anything traceable to a stage-1
(`Generate Assets …`) column is the cross-stage leak bug (Step 4a.0 catches it).

**The Outputs section of Step 1: Generate Assets is read-only input.** Stage-1's
deliverables live there and stage-2 skills may need them as input. Reading that
section is allowed and expected; executing the `Generate Assets Skill`-column skills that
produced it is not.
...
single space. So `Finalize & Publish Assets` and `finalize-&-publish-assets` match
`finalize & publish assets` (only hyphens, underscores, and whitespace collapse —
the `&` is preserved). When **setting** a status, resolve the board's actual option
by normalized match rather than writing a hard-coded literal, so the routine works
regardless of the workspace's exact spelling.
...
- "Run the publish step" / "Process all Finalize & Publish Assets cards"
- "Publish the <Playbook name> cards" (pass the Playbook name as a filter)
- Any request to batch-execute the publish step for Perkins cards
- **Triggered automatically by a Notion webhook when a card enters
  `Finalize & Publish Assets`** (single-task mode — the webhook text contains the
  page ID; see Step 3a)
...
| **Playbook name** | Optional | User may specify a playbook name to filter to. Ignored in single-task mode. If omitted and no card ID is provided, process all `Finalize & Publish Assets` cards. |
...
- The card's Status is set to **`Completed`** (transitioning it out of
  `In progress`) — the publish step has run, so the card is done.
- One run block is appended to the card body under the **Outputs section of
  Step 2: Finalize & Publish Assets**. Repeat runs stack, never overwrite.
...
- The card is **left at `In progress`** (the value set at claim) and its
  ...
  is **never** reverted to `Finalize & Publish Assets` — reverting `In progress → Finalize &
  Publish Assets` re-fires the stage-2 webhook and auto-retries a deterministic
  failure forever. It is **never** reverted to `Generate Assets` (that re-triggers stage-1
  and overwrites the reviewed stage-1 outputs) and **never** set to `Human
  Review` (a blocked stage-2 run is a retryable publish attempt, not a request to
  re-review stage-1). The card stays visibly `In progress`; a human reads the
  blocked log and re-queues it to `Finalize & Publish Assets` manually once the block is
  cleared.
- A `### Blocked` run block is appended under the **Outputs section of Step 2:
  Finalize & Publish Assets** noting the reason, so humans see failed attempts.

**The Outputs section of Step 1: Generate Assets is read-only from this
routine's perspective.** Stage-1 outputs live there and stage-2 skills may need
to read them. This routine never appends to, edits, or reorders that section —
it only reads it (passed to subagents as input) and writes to its own Outputs
section of Step 2: Finalize & Publish Assets.
...
- **Board returns zero `Finalize & Publish Assets` cards** — tell the user the board
  is clear and stop. Do not fabricate work.
- **Single card ID provided but the card is no longer `Finalize & Publish Assets`** —
  stop cleanly, report its current status (a stale webhook — a human or another
  run already moved it). Do not process it, do not append anything.
...
- **Pre-flight check fails (Step 3b)** — mark the card blocked; name the
  specific check that failed (missing Playbook relation, empty `Finalize & Publish Assets Skill`, or an
  index-pair count mismatch).
...
3. GitHub fallback — `hx-gtm/hxgtm-revops-pipelines` ref
   `feat/migrate-hx-marketing-skills` path `.claude/skills/{skill_name}/SKILL.md`,
   then `hx-gtm/hx-plugins` ref `feature/hx-marketing-design-skills` path
...
```
{"text": "Notion item status changed to Finalize & Publish Assets. ID: 344802db-20a6-80d6-8ba3-dabe1b6ba6e6. URL: <https://www.notion.so/Celent-Webinar-344802db20a680d68ba3dabe1b6ba6e6.">}
```

Extract:

- **Status** — value after `"status changed to "`, up to the next `.`. Normalize
  it (per the Stage Ownership rule) and match against `finalize & publish assets`; if it
  doesn't match, **hard-stop** — the webhook is for some other status and is
  therefore another routine's (or stage's) trigger. **This is the cross-routine
  guard:** stage-2 never processes a card whose trigger status is not
  `Finalize & Publish Assets`, even though the same webhook stream may also feed the
  stage-1 routine.
...
2. Its Status still normalizes to `finalize & publish assets`. If it has moved on, stop
   per the stale-webhook rule.
...
normalizes to `finalize & publish assets`. Never widen this filter to any other status —
a card that is not `Finalize & Publish Assets` belongs to a different stage and is out of
scope for this routine.

### Step 3b — Resolve each card's Playbook and run pre-flight
...
Read the **stage-2** wiring columns from that row as surfaced by the
**Playbook Wiring (Admin View)** view — the stage-2 (`Finalize & Publish Assets …`) columns, **not** the
stage-1 ones (those drive stage-1):

| Property | Use |
| --- | --- |
| **Playbook** (title) | Playbook name — display and optional filtering |
| **Finalize & Publish Assets Skill** | Ordered, comma-separated stage-2 skill list |
| **Finalize & Publish Assets Content Type** | Content-type hint(s) — same index order as `Finalize & Publish Assets Skill` |
| **Finalize & Publish Assets Guidance Type** | Guidance-category hint(s) — same index order as `Finalize & Publish Assets Skill` |
| **Description** | Additional creative direction for the brief |
| **Finalize & Publish Assets Custom Instructions** | Free-text, playbook-specific note appended once to the subagent prompt for this step (a **single value**, not an index-paired list — so it is **not** part of the index-position contract or the pre-flight count check below) |

**FORBIDDEN here (per Stage Ownership):** do not read for wiring, merge, or fall
back to the stage-1 `Generate Assets Skill`, `Generate Assets Content Type`, `Generate Assets Guidance Type`, or
`Generate Assets Custom Instructions` columns. They are surfaced on the same row/rollup but
belong to the stage-1 routine. Build the skill list and all index-pairing
**only** from the stage-2 (`Finalize & Publish Assets …`) columns.

**Index-position contract:** within the stage-2 columns, `Finalize & Publish Assets Skill[i]` ↔ `Finalize & Publish Assets Content Type[i]` ↔ `Finalize & Publish Assets Guidance Type[i]`.
The three lists pair by index; reordering one without the others silently
breaks wiring with no error (v3 §5). The pre-flight below catches the most
common form of that failure.

**Pre-flight checks (all must pass, per card).** Any failure → the card is
blocked with a descriptive reason naming the failed check; status reverts to
`Finalize & Publish Assets` (Step 4c). Do not launch a subagent for a card that fails
pre-flight.

1. The card's `Playbook` relation is populated.
2. The resolved Playbook row's `Finalize & Publish Assets Skill` is non-empty.
3. `Finalize & Publish Assets Content Type` has the **same comma-separated item count** as `Finalize & Publish Assets Skill`.
   A mismatch is the v3 §5 silent-failure mode — caught here, not silently.
4. `Finalize & Publish Assets Guidance Type` is either blank **or** has the same item count as `Finalize & Publish Assets Skill`.
...
For each card that passes pre-flight, assemble: card name (display only), due
date, stage-2 skill(s) (`Finalize & Publish Assets Skill`), `Finalize & Publish Assets Content Type`, `Finalize & Publish Assets Guidance Type`, Playbook
description, `Finalize & Publish Assets Custom Instructions` (the playbook's free-text note, or `None` if
blank), **full page body including the Outputs section of Step 1: Generate
Assets** (the brief plus stage-1's deliverables — see Step 4b), and page URL.

**Per-step form inputs (converted playbooks).** Newer templates collect Step 2
inputs through a Notion form rather than inline body tables: the card's `Step 2:
Finalize & Publish Assets` → `Inputs` section holds an `Add Inputs` button and a
database placed as a **full-page child** of the card (`<database inline="false">`).
When the card uses this pattern, also resolve that database: fetch the card via the
Notion MCP, read the `Step 2 Inputs` full-page-child database's id, and record it on
the manifest as `step_input_db_id`. Read the **latest** submission row and fold its
fields into the brief as the card's structured Step 2 inputs — including any
`Select Preferred LinkedIn Ad Concept(s)`-style **multi-select** (the marketer's
chosen Step 1 concepts/variants), read as the list of selected option names. File
fields stay in the database row for `download-from-notion` to fetch at skill time.
If the card still uses inline input tables, `step_input_db_id` is `None`.
...
1. The dispatch list equals the card's `Finalize & Publish Assets Skill` column contents, in order, plus
   only the chained sub-skills those skills document (and minus `upload-to-notion`,
   which the orchestrator runs in Step 4c).
2. **No** entry in the dispatch list traces back to the stage-1 `Generate Assets Skill`,
   `Generate Assets Content Type`, or `Generate Assets Guidance Type` columns.
...
includes a skill not in the Finalize & Publish Assets Skill column (<names>)` and record it in the
completion summary. This is the last line of defense before a stage-1 skill runs
inside stage-2.
...
This is safe: `In progress` is not a watched trigger (stage-2 watches `finalize &
publish assets`), so flipping to it never re-triggers a routine — the resulting
"status changed to In progress" webhook is hard-stopped by every routine's Step 3a
cross-routine guard.
...
On a `completed` result the orchestrator sets the pipeline `Status` to
**`Completed`** and `Agent Status` to **`Done`** (Step 4c); on a `blocked` result
it **leaves the card at `In progress`** and resets `Agent Status` to `not started`
(Step 4c). Either way the Step 4a claim never strands the card.
...
mode (e.g. a Puppeteer-rendering subagent emitting hallucinated environment
refusals because `SKILL.md` was never injected).

- **`subagent_type`**: `general-purpose`.
- **`skills`**: the closed skill list resolved from the card's `Finalize & Publish Assets Skill` column in
...
>
> You are a subagent dispatched from the Perkins playbook **publish-to-production**
> orchestrator. The skill(s) in your `skills:` dispatch field are injected into
> your context at startup — full `SKILL.md` content is available. Run each
> skill's documented pipeline end-to-end. Do not describe what a skill *would*
> do; execute it.
>
> **Closed skill set.** Execute ONLY the skills in your `skills:` dispatch field,
> in order. The task card surfaces other skills that belong to the **stage-1**
> pipeline (a `Generate Assets Skill` rollup on the card's properties, and the skills whose output
> already sits in the Outputs section of Step 1: Generate Assets in the card body).
> You MAY read that stage-1 Outputs section as input context, but you MUST NOT
> execute, chain to, or "helpfully" add any skill that is not in your dispatch
> list. Your skill list is closed and authoritative — running an out-of-list skill
> is the cross-stage leak bug.
>
...
> come from the routine's Setup Script, not from npm. If an exporter fails with
...
> ---
>
> You are executing the publish-to-production step of a Perkins card. Follow
...
> Use absolute local paths for any `node`/`npm`/shell command. Skills that
> render artifacts MUST run their local pipeline end-to-end.
>
...
>
> **Stage-1 outputs are part of your input.** The card brief below includes the
> card's full page body, including its Outputs section of Step 1: Generate
> Assets — the deliverables produced by the earlier stage-1 run. Each skill in
> this run knows its own input contract: extract whatever stage-1 output you
> need from that section. The orchestrator does NOT pre-extract or filter it for
> you. Treat the Step 1 Outputs section as read-only reference material — never
> write to it.
>
> **Notion file inputs.** If the brief, page body, or page Files property
> references attachments a skill needs as local inputs, resolve them via
> `download-from-notion` **before** invoking the consuming skill. Pick the
> mode that matches where the input lives:
>
> - **Page-property mode.** Run `download_from_notion.sh --api-key
>   "$NOTION_API_KEY" --page-id <card-page-id> --property <files-property>
>   --output-dir <abs-dir>`. Re-fetches the page so signed-URL TTL is never a
>   problem.
...
>   from an inline child database; each row drives one producer-skill render).
>   Run `download_child_db_rows.py --api-key "$NOTION_API_KEY" --page-id
>   <card-page-id> --child-db-name "<child-db name>" --output-dir <abs-dir>`.
>   Returns one manifest entry per row in `Order` sequence; per-row resolution
>   prefers any attached file column, falls back to a LinkedIn URL column if
>   the skill documents one, and marks the row `skipped` (not failed) if both
>   are blank — pass the `status: ok` rows on to the producer skill as its
>   input list.
> - **Form-inputs DB mode** (converted playbooks, when `step_input_db_id` is set
>   on the manifest — Step 2 inputs come from a per-step form database placed as a
...
>   so the producer skill is unchanged. A singleton file input on the row is
>   fetched with **page-property mode** pointed at the submission row's page id.
>   The non-file fields of that row (e.g. the `Select Preferred …` multi-select
>   of chosen Step 1 concepts) arrive in the brief — see `step_input_db_id` below.
...
> pass them into the consuming skill's required input fields. If a skill
> hard-fails on a missing input path, surface that error verbatim — never
...
>
> **Output handling.** Artifact-producing skills (PNGs, PDFs, etc.) MUST write
> to local disk under `{local_revops_path}/campaigns/<slug>/` and **stop there**
> — do NOT chain `upload-to-notion`; the orchestrator uploads artifacts onto the
> card. Text-only deliverables run `save-to-notion` (when a skill documents that
> chaining step) to create a working copy and a reference copy (separate Notion
> draft pages — this is not a card write). Report local artifact paths and
> `save-to-notion` URLs in `TASK_RESULT`.
>
...
> "None", ignore.
>
> {custom_instructions_2_or_none}
>
> **Card brief (full Notion page body, including the Outputs section of
...
>
> When this is set, the card's structured Step 2 inputs come from the latest
> submission row of this database (a full-page child of the card), not from inline
> input tables in the body above. Read its non-file fields as the inputs — including
> any `Select Preferred LinkedIn Ad Concept(s)`-style multi-select, which names the
> Step 1 concepts/variants the marketer chose to finalize and publish. Fetch its
> file fields via `download-from-notion` Form-inputs DB mode (see above).
>
> ---
>
...
1. **Idempotency guard.** Fetch the card's existing content under the Outputs
   section of Step 2: Finalize & Publish Assets. If a block with content
   identical to the one you are about to append already exists, STOP — do not
   append a duplicate. This catches retried dispatches.
...
3. **Append the text block.** Append the assembled block under the Outputs
   section of Step 2: Finalize & Publish Assets (create the Outputs heading,
   and the Step 2 parent heading, if either is missing — never above or inside
   the Step 1 section). This append carries text only — the artifact images are
   appended next, so they land directly beneath this run block. **Never touch
   the Outputs section of Step 1: Generate Assets — it is read-only for this
   routine.**
4. **Upload artifact images — required, not optional. The orchestrator owns
   ...
   **Positioning the image (`--after`) — required.** Without it, Notion appends
   the block at the **end of the page**, so it can land after Step 2's trailing
   blocks instead of snug under this run. Anchor every image with `--after`:
   - **Anchor** = the block id of the **last block currently in the Outputs
     section of Step 2: Finalize & Publish Assets** — normally this run's text
     block, just appended in step 3 (capture the last created block's id from the
     append response). If this run appended no text block under Outputs (e.g. a
     single artifact skill on an image-only success with no run divider), fetch
     that Outputs section and use the id of its last existing child, or the
     Outputs heading's own id if the section is otherwise empty.
   - **Multiple images** (e.g. an N-entity skill rendering multiple variants):
     anchor the first image as above; then use the `block_id` that
     `upload_to_notion.sh` returns on success as the `--after` anchor for the
     next image, so they stack in `primary: true` payload order. (Reusing one
     anchor for every image would reverse their order.)

   **Each artifact must reach the card
   as an actual embedded image block — never as a filename or local path
   written as text.** If `upload_to_notion.sh` exits non-zero or returns a
   `## Partial success — block-append failed` block, copy that block verbatim
   into the run summary — do not collapse it; it carries the
   `file_upload.id` and the manual-retry recipe.
5. **Set the card's pipeline `Status` to the option that normalizes to
   `completed`** (transitioning it out of `In progress`) and set `Agent Status` to
   the option that normalizes to `done` (releasing the Step 4a claim), both
   resolved by normalized match against the board's actual options — do not write
   hard-coded literals. The publish step has run, so the card is done.
...
1. **Leave the card's pipeline `Status` at `In progress`** (the value set at
   ...
   `not started` to release the claim. **Never revert `Status` to `Finalize &
   Publish Assets`** — reverting `In progress → Finalize & Publish Assets` re-fires the
   stage-2 webhook and auto-retries a deterministic failure forever. Never set
   `Human Review` (a blocked stage-2 run is a retryable publish attempt, not a
   stage-1 re-review request) and never revert to `Generate Assets` (that re-triggers
   stage-1 and overwrites the reviewed stage-1 outputs). The card stays visibly
   `In progress`; a human reads the blocked log and re-queues it to `Finalize &
   Publish Assets` manually once the block is cleared.
2. Append a block under the Outputs section of Step 2: Finalize & Publish Assets.
   **If the block is attributable to an artifact skill** — i.e. the dispatched
   `Finalize & Publish Assets Skill` list contains exactly ONE artifact skill, OR
   the `reason`/payload names the failing artifact skill(s) — append the single
...
of Step 2: Finalize & Publish Assets. Runs are never overwritten or reordered;
new runs append at the end.** If the Outputs section (or its Step 2 parent)
does not exist yet, create it; otherwise append the new block as the last
child of that section and do not touch existing blocks. The Outputs section of
Step 1: Generate Assets is never modified.
...
- **Completed run — group every skill under its own real `### <skill-name>` H3
  ...
  2. **`status: completed` but the skill returned zero usable `primary`
     artifacts** — no image generated (the silent-gap guard at the top of Step
     4c.4).
  ...
  `## Partial success — block-append failed` recovery block from Step 4c.4, which
  carries the `file_upload.id` and the manual-retry recipe.
...
2. **Image block(s)** (Step 4c.4) — inserted by `upload-to-notion`'s
   ...
   the Step 2: Finalize & Publish Assets Outputs section — directly beneath this
   run's text (never after the page's trailing blocks). The orchestrator never
   writes an artifact as a filename or path in the text block — the embedded
   image block is the deliverable.
...
> | --- | --- | --- | --- | --- |
> | [name] | [playbook] | Completed | [skills] | [labeled outputs] |
> | [name] | [playbook] | Completed ⚠ partial | [skills] | [labeled outputs] |
> | [name] | [playbook] | Finalize & Publish Assets — blocked: [reason] | — | — |
>
...
scanning the summary can spot a partial failure without opening the card. (A stage-2 partial
still lands on `Completed`; the `⚠ partial` marker is the signal a human watches
for, since stage-2 has no review lane after this.)
