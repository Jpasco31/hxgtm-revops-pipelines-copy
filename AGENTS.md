...
| `/trigger-dossier-batch "Account A" "Account B" ...` | Fire the Account Dossier batch **cloud routine** from a Claude session (the replacement for the old n8n trigger). Collects the account list and POSTs the routine's `/fire` payload — it does not generate dossiers itself. Reads `DOSSIER_ROUTINE_TOKEN` + `DOSSIER_ROUTINE_ID` from the env. |
| `/kb-lint [dimensions]` | Audit the KB for staleness, contradictions, drift, and unprocessed raw sources |
| `/kb-lint --group <slug> [dimensions]` | Group-scoped KB audit (staleness, contradictions, drift, raw backlog). Produces a markdown report at `outputs/kb-lint-<group>-YYYY-MM-DD.md`. Run `/kb-lint --list-groups` to see groups. |
| `/kb-update --group <slug>` | Diff raw source(s) — chat attachments, URL, inline-pasted markdown, or unprocessed files in `raw/<slug>/` — against a group's canonical KB and publish findings to the "KB - Updates Review" Notion database for team triage. Auto-batches with ≥2 inputs (parallel waves of 3). |
...
kb-lint is **group-scoped** — every run targets one group from
`.claude/skills/kb-lint/config.yaml` (e.g. `competitive`, `messaging`). The
`--group <slug>` argument is required. All 11 groups are currently active.

This scans the group's slice of the canonical KB
(`../hxgtm-mcp-server/context/`) plus its raw staging folder
(`raw/<group>/`), cross-references documents, and produces a
severity-ranked markdown report.

**Dimensions checked:**
- Freshness (stale review dates, outdated claims)
- Raw vs canon conflicts (new sources that contradict canonical content)
- Unprocessed raw backlog (pending source files, scoped to group)
...
**Requirements:**
- `--group <slug>` (required) — see `.claude/skills/kb-lint/config.yaml` for
...
with raw-canon conflicts, raw backlog table, high/medium/low severity
findings, coverage gaps, and run statistics. kb-lint does NOT publish
to Notion — for raw-vs-canon triage in Notion, use `/kb-update` instead.
...
attachments, an HTTP(S) URL argument, inline-pasted markdown, and
unprocessed `.md` files in `raw/<slug>/` (rows with `Process? = yes`
and blank `Last processed` in `INDEX.md`) — into a single input set.
...
**Requirements:**
- Codex desktop — required only for the attachment input surface. URL / inline / raw-dir paths work in the CLI too.
...
.claude/
  skills/
    ...
    kb-lint/                     # KB health audit (2 parallel subagents), markdown output
    ...
    design-system/               # hx marketing-artifact design-system reference
...
raw/                             # Raw source staging area for kb-lint (11 group folders)
scripts/
...
- This is a Codex project (not a Codex Cowork plugin). It runs via the
